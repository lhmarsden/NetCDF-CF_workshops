...
```{r}
nc <- open.nc("AeN_SeaWaterChlorophyllA_20191210_P2.nc")
print.nc(nc)
```
...
HOW CAN I
1. Look at all attributes from 1 variable
2. Look at all global attributes

We can access a variable attribute like this:

```{r}
att.get.nc(nc, "CHLOROPHYLL_A", "units")
```
...
```{r}
chla <- var.get.nc(nc, "CHLOROPHYLL_A")
depth <- var.get.nc(nc, "DEPTH")
phaeopigment <- var.get.nc(nc, "PHAEOPIGMENT")
filteredvol <- var.get.nc(nc, "FILTERED_VOLUME")
plot(chla, -depth)
```
